...
int main() {
    ...
    ROCC_INSTRUCTION_S(0, &d, 0x41); // Send destination address
    ROCC_INSTRUCTION_S(0, &c, 0x42); // Send flags
    // DSS used to block the main core.
    ROCC_INSTRUCTION_DSS(0, status, &a, &b, 9); // Wait for result (not sure)
