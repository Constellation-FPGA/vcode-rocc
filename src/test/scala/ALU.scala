...
trait ALUBehavior {
  ...
  def testAnd(aVals: Seq[BigInt], bVals: Seq[BigInt], s: Int, batchSize: Int): Unit = {
    assert(aVals.length == bVals.length, "Both input vectors MUST be the same length!")
...
class VectorAnd extends AnyFlatSpec with ChiselScalatestTester with Matchers with ALUBehavior {
  behavior of "ALU"
  val xLen = 64
  val batchSize = 1
  ...
  testData.foreach { data =>
    it should behave like testAnd(data._1, data._2, xLen, batchSize)
  }
}
