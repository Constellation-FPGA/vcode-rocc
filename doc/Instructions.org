...
The instruction follows the format shown below:
~customX rd, rs1, rs2, funct7~, where ~funct7~ is written in hexadecimal.

The compiler should automatically set the ~x~ flag bits based on the provided value of the register.
Meaning if you want to ignore a register, provide the zero register.
